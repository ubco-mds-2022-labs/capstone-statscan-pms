...
cutoff_list <- list(cquintiles, cmanual, chdbscan, cmixall, cjenks, cmclust, cpamkmeans )

names(cutoff_list) <- c("cquintiles", "cmanual", "chdbscan", "cmixall", "cjenks", "cmclust", "cpamkmeans" )

# initialize df with first
df_cutof <- cbind(rep(names(cutoff_list)[1], length(cutoff_list[[1]][["PMS_prox_idx_emp"]])), cutoff_list[[1]][["PMS_prox_idx_emp"]])

# add
for (algo in 2:length(cutoff_list)){
  # add to temporary
  # add to full
  df_cutof <- rbind(df_cutof, cbind(rep(names(cutoff_list)[algo], length(cutoff_list[[algo]][["PMS_prox_idx_emp"]])), cutoff_list[[algo]][["PMS_prox_idx_emp"]]))
}
...
ggplot(df_cutof,aes(x = as.numeric(cutoff),y = algo))+geom_point(shape="|",size=4) + xlim(0,1) + labs(title = "Employment", x = "Cutoffs", y = "Method")

# maybe want overlaying in different colours?

ggplot(df_cutof,aes(x = as.numeric(cutoff), y = 0, color = algo))+geom_point(shape="|",size=4) + xlim(0,1) + labs(title = "Employment", x = "Cutoffs", y = "Method")
```
