---
title: "Manual cutoffs: density(n=1024)"
...
In this copy, I changed the `density` 'n' value from the default `n=512` to `n=1024`. 'n' in the `density` function is the number of equally spaced points at which the density is to be estimated.

Tldr result: n=1024 hows same number of groups as n=512, and cutoff values very similar if not identical to 2 s.d. Need closer analysis to see if difference between the two follow a pattern or just random.
...
for (amen in amenities){
  ...
  count <- count+1
  amen_list <- list()
  amen_list[[1]] <- density(master_log[,amen], n=1024, na.rm = T) # list of densities
  d_amen <-  density(master_log[,amen], n=1024, na.rm = T)
...
## Summary
Should find a way to compare with when n=512 in same doc to see how different the cutoff values are, see if diff number of groups at all. 'Manual' comparison shows same number of groups, and cutoff values very similar if not identical to 2 s.d.
...
```{r, eval = F}
d_amen <-  density(master_log[,8],n=1024, na.rm = T)
