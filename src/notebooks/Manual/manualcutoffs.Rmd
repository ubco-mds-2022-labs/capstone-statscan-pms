...
Similarly to Pharmacies, Childcare has indiscernible maxima and minima in some areas, as seen in Figure XX. Figure XX shows the difference again, and we can see that the same threshold of 0.001 may be appropriate. Figure XX shows the density with the values with a difference beneath this threshold removed, showing 2 cutoffs for 3 groups.
...
```{r eval = F, fig.cap="difference between density value of a maxima-minima pairs, threshold = 0.001"}
plot(big_list[[amen_num]][["maxmindiff"]], ylab = "Difference", xlab= "")
abline(h = 0.001, lty=2, col = "grey")
```

```{r eval = F, fig.cap="density plot with suggested cutoff points in blue"}
# removing minima that aren't above threshold
# add include row in df
threshold <- 0.001
big_list[[amen_num]][["df_minmax"]][[1]] <- big_list[[amen_num]][["df_minmax"]][[1]] %>% mutate(include = ifelse(index %in% which(big_list[[amen_num]][["maxmindiff"]] < threshold), 0,1))
