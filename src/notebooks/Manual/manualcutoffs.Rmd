...
for (amen in amenities){
  ...
  names(amen_list) <- c("density", "maxima", "minima", "maxmindiff", "plot", "nopt")
...
In the case of Pharmacies, there are many technical minima and maxima in an area that visually seems flat and has overall very low density. There is no doubt that these areas are not indicative specifically of density sparse regions, as the whole area is density sparse. The difference in density between the first maxima and the first minima, for example, is `r first_diff`, which is very small compared to the 8th (the 2nd visually discernible peak): `r eight_diff`. This may suggest that we should only use as cutoffs the minima that have a threshold difference with the neighbouring maxima, where the difference is as a base at least 0.0001.
...
The following plot shows the difference between the density value of maxima-minima pairs (unidirectional). We see that for Pharmacies, the difference in the first 6 pairs is very small, as we can tell from the previous plot. 
...
```
```{r}
plot(big_list[["PMS_prox_idx_pharma"]][["maxmindiff"]], ylab = "Difference")
```
