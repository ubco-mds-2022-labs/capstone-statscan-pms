...
knitr::opts_chunk$set(echo = TRUE, cache = T, include = F, message = F, warning = F)
library(dplyr)
library(factoextra)
library(ggplot2)
library(tidyverse)
...
We see that Saskatchewan has the most DBs included with a population of zero, followed by Ontario, Quebec, and Alberta. 
```{r}
...
In this violin plot, we see that the highest densities of proximity values lie below 0.12 for all amenities. We see that the amenities with the highest distribution density closer to 0 are health, then employment, then transit. Given that health and employment have the least amount of missing values
```{r}
