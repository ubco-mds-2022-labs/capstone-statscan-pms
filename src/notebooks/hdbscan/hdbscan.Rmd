...
algo = function(dataset, amen_name, num=NULL){
  ...
  res = hdbscan(as.matrix(dataset), minPts = 500)
...
do_everything = function(dataset, amen_name, num){
  ...
  barplot(as.matrix(table(clusts)/length(clusts)), col=sort(unique(clusts)), ylim=c(0, 4), horiz=TRUE, xlab='Proportion')
