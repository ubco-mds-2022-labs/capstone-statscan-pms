...
master_sample_log <- data.frame(matrix(nrow = nrow(master_sample), ncol = ncol(master_sample)))
...
Missing values are managed, without any pre-processing, by the model used to cluster with the assumption that
...
```{r}
#master function
do_everything_single = function(dataset, num){
  
  # remove NA values
  clust_data = dataset[!is.na(dataset),]
...
```{r}
lst = do_everything(master_sample_wo_na, 'PMS_prox_idx_emp', 2:6)
