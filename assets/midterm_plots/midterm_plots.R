...
sil_df = data.frame(squintiles, smixall, shdbscan, skpam, smclust)
...
dunn_df = data.frame(squintiles, smixall, shdbscan, skpam, smclust)
...
cal_df = data.frame(squintiles, smixall, shdbscan, skpam, smclust)
...
dav_df = data.frame(squintiles, smixall, shdbscan, skpam, smclust)
...
cquintiles = list(PMS_prox_idx_emp = c(0.012, 0.021, 0.036, 0.069), 
...
for (i in all_coef){
	...
	temp = ggplot(data, aes(x=amenities, y=value, fill=name)) + 
	  ...
	  ylab(all_coef_names[counter]) + xlab('') +
	  scale_fill_discrete(name = "Algorithm    ",
		                  labels = c("Quintiles", "MixAll", "HDBSCAN", "PAM k-means", "MCLUST"),
