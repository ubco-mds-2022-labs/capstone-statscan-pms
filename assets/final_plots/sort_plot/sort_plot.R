...
for(i in amenities){
  ...
  plt = ggplot(df, aes(x=index, y=value)) + geom_line(aes(col=1)) + 
...
layout_mat = rbind(c(1:4),
                    c(5:8),
                    c(9:11, 11))
sort1 = do.call(grid.arrange,list(grobs=p, layout_matrix=layout_mat,top ="Sort Plots by Amenity"))
...
layout_mat = rbind(c(1:4),
                    c(5:8),
                    c(9:11, 11))
sort2 = do.call(grid.arrange,list(grobs=p, layout_matrix=layout_mat,top="Log-transformed Sort Plots by Amenity"))
